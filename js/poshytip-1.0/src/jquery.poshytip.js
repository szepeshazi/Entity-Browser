...
(function($) {
	...
	$.Poshytip.prototype = {
		...
		hide: function() {
			if (this.disabled || !this.$tip.data('active') || this.$tip.animating)
				return;
		...
		refresh: function(async) {
			...
			if (async && !this.$tip.animating){
			     this.$tip.animating = true;
			     var theTip = this.$tip;
			     this.$tip.css(currPos).animate({left: this.pos.l, top: this.pos.t}, 200, function(){ theTip.animating=false; });
			} else {
			     this.$tip.css({left: this.pos.l, top: this.pos.t});
			}				
		},
		display: function(hide) {
			if (!this.opts.beforeDisplay(this.$elm)) { return; }
	...
	$.fn.poshytip.defaults = {
		...
		beforeDisplay:		function() { return true; }
	};
